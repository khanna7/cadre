...
STD_OUTPUT=$INSTANCE_DIRECTORY/output.txt

PARAM_FILE=$4

source $EMEWS_ROOT/scripts/${SITE}.sh

# source $EMEWS_ROOT/envs/midway2.sh
export PYTHONPATH=$EMEWS_ROOT/..
...
arg_array=( "-m" "p2m4py" "$PARAM_FILE" "-p" "$PARAM_LINE" )
...
srun --exclusive -n 1 -c 1 python3 -m p2m4py "$PARAM_FILE" -p "$PARAM_LINE"
