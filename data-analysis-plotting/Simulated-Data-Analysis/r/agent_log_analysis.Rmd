...
```{r}
agent_dt <- fread("~/code/cadre/python/output/agent_log_11.csv")
str(agent_dt)
...
# Create the plot
p <- ggplot() +
  geom_line(data = current_smoker_df, aes(x = Time, y = Proportion, color = "Released")) +
  geom_line(data = general_population, aes(x = Time, y = Proportion, color = "General")) +
  scale_color_manual(values = c("Released" = "blue", "General" = "red")) +
  scale_x_continuous(breaks = time_periods, labels = labels) +
  scale_y_continuous(breaks = seq(0, 1, 0.2), limits = c(0, 1)) +
  labs(
    title = "",
